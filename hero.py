class Hero():
    ...
    # Метод для будування блоків (в залежності від ігрового режиму)
    def build(self):                      
       angle = self.hero.getH() % 360
       pos = self.look_at(angle)
       if self.mode:
    ...
    # Метод для видалення блоків (в залежності від ігрового режиму)
    def destroy(self):                      
       angle = self.hero.getH() % 360
       pos = self.look_at(angle)
       if self.mode:
    ...
    def accept_events(self):
        ...
        # встановлення блоку звичайної текстури 
        base.accept("1", self.land.set_block)
        # встановлення блоку текстури каменю
        base.accept("2", self.land.set_stone)
        # встановлення блоку текстури цегли
        base.accept("3", self.land.set_brick)
        # встановлення блоку текстури дерева
        base.accept("4", self.land.set_wood)
